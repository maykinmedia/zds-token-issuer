...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@bitbucket.org:maykinmedia/token_supplier.git
       $ cd token_supplier
...
``src/token_supplier/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    $ python src/manage.py test token_supplier
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting token_supplier_db_1 ... done
       Starting token_supplier_web_1 ... done
   ...
   .. code-block:: bash
       ...
       $ docker logs -f token_supplier_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it token_supplier_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it token_supplier_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/token_supplier/
...
environment variables. See ``src/token_supplier/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t token_supplier
    $ docker run \
        -p 8000:8000 \
        -e DJANGO_SETTINGS_MODULE=token_supplier.conf.docker \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
        -e DATABASE_HOST=... \
        --name token_supplier \
    ...
    $ docker exec -it token_supplier /app/src/manage.py createsuperuser
